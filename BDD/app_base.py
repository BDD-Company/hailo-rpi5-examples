...
# PyGObject keeps the wrapped bytes alive until the buffer is freed (then this no-op runs).
...
def picamera_thread(
        ...
        target_fps = 30,  # makes appsrc caps match the pipeline's 30/1 (no videorate duplication)
    ...
    with Picamera2(tuning=f"/usr/share/libcamera/ipa/rpi/pisp/{camera_model}_noir.json") as picam2:
        if picamera_config is None:
            ...
            # 1280x720 RGB frame, but the old config also allocated an identical, UNUSED
            # `lores` stream — doubling ISP output bandwidth for nothing (and the camera
            # was throughput-limited to ~15 fps as a result). Configure just `main`.
            # buffer_count is trimmed from the preview default (4) to 3 to shorten the
            # camera pipeline depth (fewer frames in flight => lower capture latency);
            # each request is released immediately after copying, so 3 is sufficient.
        ...
        emit_time_accum = 0.0  # subset of loop_proc spent inside appsrc push-buffer (backpressure)
        while True:
...
#            })
            ...
            # Convert BGR (picamera2 'RGB888' byte order) -> RGB.
            ...
            # references frame_bytes directly (transfer none) and keeps it alive by taking
            # it as user_data with a destroy-notify (measured ~2.5x faster than
            # new_wrapped). NOTE: true zero-copy straight from the numpy array is NOT
            # possible via PyGObject — it marshals the array param as a sequence of ints —
            # so the single tobytes() copy is unavoidable here.
