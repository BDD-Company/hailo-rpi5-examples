...
+ write extra logs into the px4's persistent ulog — DONE 2026-07-13 (branch `feat/px4-ulog-trace`). One MAVLink `DEBUG_FLOAT_ARRAY` named `BDD` at 5 Hz (`ulog_trace:` config section, absent/`enabled: false` = off), which PX4 republishes as the `debug_array` uORB topic and writes into its own log. New `ulog_trace.py` (`FrameOutcome` + `pack_history` + `UlogTraceSample` + `TraceAccumulator` + `UlogTracer`); `DroneMover.send_debug_array()` + `last_attitude_command()`; two `note()` sites in `drone_controller` (after the command has gone out, so ZERO added capture→command latency; and from the STARVED path, so the trace keeps beating with a rising starvation count when the vision pipeline dies instead of just falling silent).
  **A record is an INTERVAL SUMMARY (format v2), not a snapshot.** The loop runs ~20–30 fps and we send at 5 Hz, so one record covers ~5–6 frames. A snapshot would discard the other five AND misreport — one miss right after a good detection would log as "no detection", erasing it. So each record carries, for the whole interval SINCE THE LAST SENT RECORD: the two starvation COUNTS, a packed PER-FRAME HISTORY (12 frames, 4 bits each, 2 slots, newest first), and the LAST VIABLE DETECTION of the interval. `pd_p` + command stay momentary (the values in force), and the history says how many frames ago the detection was, so there is no ambiguity. The accumulator resets on DISPATCH, not when a record is merely due — so a send skipped behind a stalled link extends the interval instead of losing it. 16 of 58 slots used.
  History encoding: 4 bits/frame, NOT 2. Four outcomes (`NO_FRAME`/`NO_DETECTIONS`/`NO_VIABLE`/`VIABLE`) would fit in 2 bits, but then a partially-filled slot's padding zeros are indistinguishable from real `NO_FRAME` starvation and every short interval reads as a dead pipeline — so state 0 = `NO_DATA`. 24 bits/slot is the max: float32 holds integers exactly only to 2^24. Widths are FIXED BY THE FORMAT VERSION.
  Replay of the 39 m/s UAE dive proves the aggregation conserves everything: 1012 counted in slots 2+3 + 643 VIABLE frames in the histories = **1655 = exactly the number of control-loop iterations**, none lost, none double-counted. 20 records carry a detection whose newest frame was a miss (all 20 would have logged det=0 under the snapshot design); records carrying a detection 141 → 161.
