...
Whenever list of tasks is updated and the task description takes longer that couple of sentenses, move it to the `Further refinement of tasks` section.
...
+ write extra logs into the px4's persistent ulog — DONE 2026-07-13 (branch `feat/px4-ulog-trace`). One MAVLink `DEBUG_FLOAT_ARRAY` named `BDD` at 5 Hz (`ulog_trace:` config section, absent/`enabled: false` = off), which PX4 republishes as the `debug_array` uORB topic and writes into its own log. New `ulog_trace.py` (`FrameOutcome` + `pack_history` + `UlogTraceSample` + `TraceAccumulator` + `UlogTracer`); `DroneMover.send_debug_array()` + `last_attitude_command()`; two `note()` sites in `drone_controller` (after the command has gone out, so ZERO added capture→command latency; and from the STARVED path, so the trace keeps beating with a rising starvation count when the vision pipeline dies instead of just falling silent).
  **A record is an INTERVAL SUMMARY (format v3), not a snapshot.** The loop runs ~20–30 fps and we send at 5 Hz, so one record covers ~5–6 frames. A snapshot would discard the other five AND misreport — one miss right after a good detection would log as "no detection", erasing it. So each record carries, for the whole interval SINCE THE LAST SENT RECORD: the two starvation COUNTS, a packed PER-FRAME HISTORY (21 frames, oldest first, with an explicit frame count), and the LAST VIABLE DETECTION of the interval. `pd_p` + command stay momentary (the values in force). The accumulator resets on DISPATCH, not when a record is merely due — so a send skipped behind a stalled link extends the interval instead of losing it. 16 of 58 slots used.
  History encoding (slots 4–5, FIXED BY THE FORMAT VERSION): slot 4 = [count: bits 0-5][9 outcomes × 2 bits]; slot 5 = [12 outcomes × 2 bits]. 2 bits/frame suffices ONLY because the count is explicit — without it, padding zeros are indistinguishable from real `NO_FRAME` starvation (`NO_FRAME` *is* 0) and you would need a 5th `NO_DATA` value at 4 bits (that was v2, and it cost half the depth). 24 bits/slot is the ceiling: float32 holds integers exactly only to 2^24, and a rounded bitfield decodes to garbage rather than an obviously-wrong number. Overflow (interval > 21 frames) discards the LATEST frames that don't fit; the counts still cover everything, so it loses detail, never data.
  **`rate_hz: 0` = UNCAPPED** — one record per control-loop iteration, one frame each. Distinct from "off" (absent section / `enabled: false`).
  Verified that frames-per-record tracks BOTH the log rate (explicit) and the control-loop rate (implicit — what thermal throttling does to us), by sweeping both against the 39 m/s UAE dive and decimating its frames to fake a throttled loop: 20fps@5Hz → 5 frames/rec, 10fps@5Hz → 3, 6.7fps@5Hz → 2; 20fps@2Hz → 11, 20fps@20Hz → 2, 20fps@0Hz → 1. **At every combination the accounted-for iterations equal the frames the loop ran**, minus only the final interval still in the accumulator at stop — nothing lost between records, nothing double-counted. (A record covers floor(fps/rate)+1 frames: the frame that TRIGGERS the send is part of the interval it reports.) The counted history round-trips through the real PX4 intact, including a 13-frame history spanning both slots.
  ...
  * **No FC provisioning needed.** The rig's PX4 already has `SDLOG_PROFILE = 1057` (= 1024 + **32** + 1), so the debug bit is set. At startup `DroneMover._check_ulog_debug_logging()` reads it and GATES the tracer: if the bit is off it disables tracing (send_fn=None null-object) and warns, rather than streaming messages PX4 would silently drop — but the launch always PROCEEDS (a disabled trace never blocks the flight). An unreadable param leaves tracing ON (a transient link glitch must not kill a forensic feature). The app never writes FC params; `BDD/enable_px4_ulog_debug.py` is the run-before-flying provisioning (sets the bit, tells you to reboot — PX4 reads SDLOG_PROFILE only at boot, so it CANNOT be enabled for the current session; idempotent, `--dry-run`). Note `SDLOG_MODE = 0` — PX4 logs only while ARMED, which is exactly the crash window.
  * The detection block goes LAST and absent = 0, so MAVLink 2's trailing-zero trim shrinks a no-detection record: **85 B/msg measured, vs 264 B untrimmed** (~375 B/s at 5 Hz). The float16/bit-packing the task sketched is unnecessary for the VALUES — 58 slots is far more than the 16 needed. (It does earn its keep for the per-frame history, where the alternative is one slot per frame.)
  ...
  STRESS-TESTED unrate-limited on the rig (2026-07-13), 5 / 20 / 50 / 100 Hz and unlimited: the control loop is untouched (simulated 20 Hz tick lateness p50 0.54 ms quiet → 0.60 ms at one-message-per-frame), telemetry holds 50.0 Hz exactly even under a 2200 msg/s flood, and PX4 itself receives 1:1 up to 2221 msg/s. **The link is NOT 1 Mbps** — it is USB CDC-ACM, so the `:1000000` baud in the connection string is ignored; 2221 msg/s is ~1.95 Mbps, which a real 1 Mbps UART could not carry. **The bottleneck is the FC's LOGGER, and it fails SILENTLY**: measured by the growth of the real `.ulg` on the FC's SD, 20 Hz adds exactly +5.0 kB/s (= 20 × 256 B, all persisted), but an unlimited 2644 msg/s flood adds only +49.8 kB/s ≈ **195 records/s — it discards 93% while reporting `dropouts: 0`** (it polls the uORB topic slower than we publish and keeps only the latest sample, so these are not buffer overruns and nothing warns you). Per-frame logging is therefore safe and fully logged; unlimited is not dangerous, just pointless. The config's 50 Hz cap sits inside the fully-logged region — raising it means re-measuring that ~195/s ceiling.
...
- deprecated CLI alias `--switch-test-s` (renamed to `--test-switch-s` on 2026-07-09): DELETE the alias and `_pop_deprecated_value()` after **2026-09-30**. Kept so runbooks/skills pinned to older checkouts keep running. Test/verification-harness flags are `--test-*`; run-mode flags (`--DEBUG`, `--no-record`, `--preview`, `--vision-only`) are not.
